...
class JsspAlgorithm:
    ...
    def save_pareto_front(self, log_path, folder_name: str):
        """Function for saving the schedule Gnatt images of the pareto front solutions to disk.
            Additionally the paretofront individuals are serialized to the same file location
        ...
        folder_name : str
            Name added tot he image folder name
        """
        # Save the schedules of the pareto front in the log folder
        img_folder = os.path.join(log_path, f"pareto_front_{folder_name}")
        if not os.path.exists(img_folder):
            os.mkdir(img_folder)
        
        # List for storing the individuals in pareto front
        front_list = []
        # Obtaining the index range in the population of the best front (pareto front)
        cur_pareto_front = self.pop_object.get_front_range(0)
        for i in range(cur_pareto_front[0], cur_pareto_front[1], 1):
            # For each individual, create the Gnatt image and save it to the front list
            cur_img = self.pop_object.R[i].schedule.get_image()
            front_list.append(self.pop_object.R[i])
            # Convert image matrix to Image object
            im = Image.fromarray(np.uint8(cur_img*255)).convert('RGB')
            # Current image path is created with the index in the pareto front as id
            cur_img_path = os.path.join(img_folder, f"Schedule_{i}.png")
            # Saving the image to disk
            im.save(cur_img_path)
        
        # Saving individual objects fromt he pareto front to disk
        cur_path = os.path.join(img_folder, "pareto_front.pkl")
        with open(cur_path, "wb") as cur_dump_file:
            pickle.dump(front_list, cur_dump_file)

    def save_objective_space_plot(self, log_path: str, figure_name: str, folder_name: str, minimum_opacity: float=0.5, color_mapper: str="plt.cm.inferno"):
